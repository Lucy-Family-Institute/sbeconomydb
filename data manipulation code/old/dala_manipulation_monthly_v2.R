...
emp <- bind_rows(unemp_job_openings_sb,unemp_job_openings_elk, unemp_job_openings_oth) %>%
  ...
  arrange(dt) %>%
  mutate(last_yr_value=lag(value,12),yoy_change=(value-last_yr_value)/last_yr_value, # this assumes that data is available for all consecutive past months!
...
sb_elk_ces_supersectors$dt %>% max() # "2022-07-01" # latest date
...
laus_select_areas$dt %>% max() # "2022-07-01"
...
yr_mnth <- sprintf('%s%s', eg[,1], eg[,2]) %>% sort() %>% head(x,n=-5) # lost last 5 months of 2022 for which data is not available
...
sb_evictions$week_date %>% max() # "2022-07-31"
...
home_prices_in$dt %>% max() # "2022-07-31"

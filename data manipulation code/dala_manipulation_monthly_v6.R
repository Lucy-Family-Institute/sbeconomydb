...
max(snap_tanf_df$dt) # "2023-03-01"
...
in_series <- lapply(lst, function (x) {
  bls_api(x,
          startyear = 2004, endyear = 2023, registrationKey = "82dd32248e774edb8acb7b9c0efb800c") 
}) %>% bind_rows() %>% mutate(value=value*1000) # values are in thousands
...
sb_elk_ces_supersectors$dt %>% max() # "2023-03-01" # latest date
...
sb_elk_ces_sup$dt %>% max() # "2022-03-01"
...
in_series <- lapply(selected_series_sb_elk_msa_wage$series_id, function (x) {
  bls_api(x,
          startyear = 2004, endyear = 2023, registrationKey = "82dd32248e774edb8acb7b9c0efb800c") 
}) %>% bind_rows()
...
sb_elk_ces_supersectors_wage <- in_series %>%
  filter(period!="M13") %>% # annual average
  mutate(dt=mdy(paste0(substr(period,2,3),"-01-",year))) %>%
  left_join(selected_series_sb_elk_msa_wage, by=c("seriesID"="series_id")) %>% # add text
  select(state_code,area_code,supersector_code,industry_code,industry_name,area_name,dt,hourly_wage=value)

sb_elk_ces_supersectors_wage$dt %>% max() # "2022-03-01" # latest date
...
# area types
laus_series <- read_csv("raw data/laus_series.csv")

select_series <- laus_series %>% filter(area_type_code=="B", srd_code==18)

laus_in <- bls_api(select_series$series_id,
          startyear = 2004, endyear = 2023, registrationKey = "82dd32248e774edb8acb7b9c0efb800c") 

laus_select_areas <- laus_in %>%
  filter(period!="M13") %>% # annual average
  mutate(dt=mdy(paste0(substr(period,2,3),"-01-",year))) %>%
  left_join(select_series, by=c("seriesID"="series_id")) %>% # add text
...
sb_evictions$week_date %>% max() # "2023-04-02"
...
home_prices_in$dt %>% max() # "2022-03-31"
...
fhfa_hpi_in$yr_q %>% max() # "2022-10-01 should be fourth quarter
